...
class BACnetDevice(models.Model):
    bacnet_device = models.OneToOneField(
    ...
    device_type = models.PositiveSmallIntegerField(
        default=0,
        help_text="Set a local bacnet device to discover remote"
    ...
    port = models.CharField(
        default="47808", max_length=400, help_text="for IP default port is 47808"
    )
    bacnet_local_device = models.ForeignKey(
        Device,
        null=True,
        blank=True,
    ...
    remote_devices_discovered = models.CharField(
        default="",
        max_length=300,
        blank=True,
        null=True,
    ...
    remote_devices_variables = models.CharField(
        default="",
        max_length=2000,
        blank=True,
        null=True,
    ...
    fieldsets = (
        (None, {"fields": ("bacnet_device", "device_type", "ip_address")}),
        (
...
class BACnetVariable(models.Model):
    bacnet_variable = models.OneToOneField(
...
class BACnetVariableProperty(models.Model):
    ...
    def __str__(self):
        return (
            self.bacnet_variable.bacnet_variable.name
            + "-"
            + self.id_choices[self.property_id][1]
        )
